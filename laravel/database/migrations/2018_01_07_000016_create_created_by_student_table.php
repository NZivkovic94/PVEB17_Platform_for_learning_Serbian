...
class CreateCreatedByStudentTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create($this->set_schema_table, function (Blueprint $table) {
            $table->engine = 'InnoDB';
            $table->increments('student_id_student');
            $table->integer('lesson_id_lesson');
            ...
            $table->index(["lesson_id_lesson"], 'fk_student_has_lesson_lesson2_idx');

            $table->index(["student_id_student"], 'fk_student_has_lesson_student2_idx');


            $table->foreign('student_id_student', 'fk_student_has_lesson_student2_idx')
                ->references('id_student')->on('student')
                ->onDelete('no action')
                ->onUpdate('no action');

            $table->foreign('lesson_id_lesson', 'fk_student_has_lesson_lesson2_idx')
